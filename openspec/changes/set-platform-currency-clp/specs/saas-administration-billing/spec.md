...
El sistema SHALL administrar planes SaaS, limites de alumnos y usuarios, periodo de prueba, precio, moneda, ciclo, estado de cobro y eventos del proveedor de pagos. Cuando no se indique una moneda para un nuevo plan o suscripcion SaaS, SHALL usar CLP como moneda predeterminada.
...
#### Scenario: Crear plan SaaS sin moneda explicita

- **WHEN** un administrador crea un plan SaaS sin modificar la moneda predeterminada
- **THEN** el plan se guarda con moneda CLP

#### Scenario: Mantener suscripcion historica

- **WHEN** se consulta una suscripcion existente denominada en una moneda distinta de CLP
- **THEN** el sistema conserva su moneda e importes originales y no realiza conversion automatica
...
El sistema SHALL mostrar MRR, ARR, ARPA, churn, conversion de trial, cobrado, pendiente y tenants en mora con periodo, moneda y definicion de calculo visibles. SHALL presentar los importes CLP con locale `es-CL` y mantener indicadores separados por moneda.
...
#### Scenario: Consultar indicadores con multiples monedas

- **WHEN** un periodo contiene suscripciones en CLP y en otra moneda
- **THEN** el dashboard muestra una seccion o filtro independiente por moneda y no convierte ni suma importes entre ellas
